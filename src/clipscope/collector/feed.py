...
Usage:
    uv run douyin feed                # Collect once
    uv run douyin feed --loop         # Continuous (every 5 min)
    uv run douyin feed --loop --interval 10  # Every 10 min
"""
...
from clipscope.crawler.douyin.web.models import BaseRequestModel
from clipscope.crawler.douyin.web.utils import BogusManager
...
async def collect_once():
    ...
    logger.info("[FeedCollector] Collecting feed data...")
    ...
    if new_items:
        ...
        logger.info("Added %d new (dedup %d)", len(new_items), len(items) - len(new_items))
    else:
        logger.info("No new items (%d total, all already exist)", len(items))
...
async def main(args: argparse.Namespace | None = None):
    ...
    if args.loop:
        logger.info("[FeedCollector] Continuous mode, every %d min", args.interval)
        logger.info("[FeedCollector] Press Ctrl+C to stop")
        while True:
            await collect_once()
            logger.info("[FeedCollector] Waiting %d min for next collection...", args.interval)
            await asyncio.sleep(args.interval * 60)
    else:
        await collect_once()
        logger.info("[FeedCollector] Done")
