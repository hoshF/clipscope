...
Usage:
    uv run douyin comments <user_profile_url>

Example:
    uv run douyin comments "https://www.douyin.com/user/MS4wLjABAAAA..."
...
def get_user_dir(sec_user_id: str, nickname: str = "") -> str:
    ...
    # fallback: use sec_user_id prefix when no nickname
...
async def fetch_all_comments(
    ...
    while has_more and len(all_comments) < max_count:
        ...
        try:
            ...
            logger.info(f"    Page {page} fetch failed: {e}")
            break
...
async def fetch_all_replies(
    ...
    while has_more:
        ...
        try:
            ...
            logger.info(f"      Replies page {page} failed: {e}")
            break
...
async def fetch_user_profile(crawler: DouyinWebCrawler, sec_user_id: str) -> dict:
    ...
    try:
        ...
        logger.info(f"  Failed to fetch user info: {e}")
...
async def fetch_all_posts(
    ...
    while has_more:
        ...
        try:
            ...
            logger.info(f"  Page {page}: {len(aweme_list)} posts (total {len(all_videos)})")
            ...
            logger.info(f"  Failed to fetch posts page {page}: {e}")
            break
...
Examples:
  ...
  uv run douyin comments "https://..." --sync
        """,
    )
    ...
    logger.info("Douyin Comment Collector")
    logger.info("=" * 60)
    logger.info("User sec_user_id: %s", sec_user_id)
    ...
    logger.info(
        "   Followers: %s  Following: %s  Posts: %s",
    ...
    for idx, video in enumerate(all_videos, 1):
        ...
        if aweme_id in processed_videos:
            if args.all:
                ...
                if not has_new:
                    ...
                    skipped += 1
                    continue
                logger.info("    Found new comments, re-collecting...")
        ...
        logger.info(f"  [{idx}/{total_videos}] Collecting comments: {aweme_id} - {desc}")
        ...
        try:
            ...
            logger.info(f"    Collection failed: {e}")
    ...
    logger.info(f"   Target user: {nickname}")
    logger.info(f"   Total posts: {total_videos}")
    logger.info(f"   Collected: {len(processed_videos)} (skipped {skipped})")
    ...
    logger.info(f"   Unique commenters: {stats['total_commenters']}")
    logger.info("   IP distribution Top5: ", end="")
    top_ips = list(stats["ip_distribution"].items())[:5]
    logger.info(", ".join(f"{k}={v}" for k, v in top_ips))
    logger.info("   Top commenter: ", end="")
    if stats["top_commenters"]:
        tc = stats["top_commenters"][0]
        logger.info(f"{tc['nickname']} ({tc['count']} comments)")
    logger.info(f"   Data saved to: {user_dir}")
    logger.info("=" * 60)
