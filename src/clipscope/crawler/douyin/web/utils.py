...
from clipscope.crawler.douyin.web.xbogus import XBogus as XB
from clipscope.crawler.douyin.web.abogus import ABogus as AB

from clipscope.crawler.utils.api_exceptions import (
    APIError,
    APIConnectionError,
    APIResponseError,
    APIUnavailableError,
...
from clipscope.crawler.utils.logger import logger
from clipscope.crawler.utils.utils import (
    gen_random_str,
    get_timestamp,
    extract_valid_urls,
    split_filename,
            ...
            try:
                ...
                if len(msToken) not in [120, 128]:
                    raise APIResponseError(
                        "响应内容：{0}， Douyin msToken API 的响应内容不符合要求。".format(msToken)
                    )
