...
def analyze_profile(videos: list) -> dict:
    ...
    category_pct = {k: round(v / total * 100, 1) for k, v in 
                    sorted(category_scores.items(), key=lambda x: -x[1])}
    ...
    inferred_profile["内容形式偏好"] = f"视频 {video_pct:.0f}% / 图集 {type_counter.get('图集', 0)/total*100:.0f}%"
    
    # 作者偏好
    big_creator = follower_ranges["100万+"] + follower_ranges["10-100万"]
    inferred_profile["创作者偏好"] = f"大V占 {big_creator/total*100:.0f}% / 小创作者占 {(total-big_creator)/total*100:.0f}%"
...
async def main():
    ...
    for i in range(batches):
        ...
        try:
            ...
            print(f"   ✅ 第 {i+1}/{batches} 批: 获取到 {len(videos)} 条 (累计 {len(all_videos)})")
